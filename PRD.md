...
- **CNN Fear & Greed buy/sell** — same shape as RSI but driven by CNN's daily F&G index. Params: `buy_below` (default 25), `exit_above` (default 75), `index_type` (default `fear_greed`). Two data sources are selectable via `--fg-source`: `cnn` (default, ~5.5 years from 2020-09-18, includes component sub-indices) and `whit3rabbit` (community CSV mirror reaching back to 2011-01-03, headline score only).
