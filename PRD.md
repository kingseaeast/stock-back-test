...
- **CNN Fear & Greed buy/sell** — same shape as RSI but driven by CNN's daily F&G index. Params: `buy_below` (default 25), `exit_above` (default 75), `index_type` (default `fear_greed`). Note: CNN's public endpoint serves only ~250 trading days (~1 year) of historical data, so backtest windows for this strategy are limited to roughly the last year.
